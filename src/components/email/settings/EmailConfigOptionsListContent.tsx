...
import type { EmailConfigCategory, EmailConfigOption } from '@/lib/api';
import { getEmailConfigCategoryMeta } from '@/lib/email/settings-navigation';

interface EmailConfigOptionsListContentProps {
  category: EmailConfigCategory;
}

export default function EmailConfigOptionsListContent({
  category,
}: EmailConfigOptionsListContentProps) {
  const router = useRouter();
  const toOrgPath = useOrgPath();
  const meta = getEmailConfigCategoryMeta(category);
  const { options, isLoading, isDeleting, error, deleteOption } =
    useEmailConfigOptions(category);
  const [optionToDelete, setOptionToDelete] = useState<EmailConfigOption | null>(
    null,
  );
  ...
  return (
    <Stack spacing={3}>
      <SettingsSubPageHeader
        overline="Email Configuration"
        title={meta.pluralLabel}
        description={meta.description}
        parentBack={{ href: '/email/settings', label: 'Email Configuration' }}
          ...
          {!isLoading && options.length === 0 ? (
            <Box className="px-6 py-14 text-center">
              ...
              <Typography variant="body2" color="text.secondary" className="mt-2">
                Create your first {meta.label.toLowerCase()} for the campaign wizard.
              </Typography>
              <Button
                component={Link}
                href={toOrgPath(`${meta.href}/new`)}
            ...
            <DataTable<EmailConfigOption>
              tableId={`email-config-${category}`}
              rows={options}
              getRowId={(option) => option.id}
              excludeFields={['id', 'category']}
              isLoading={isLoading}
              searchPlaceholder={`Search ${meta.pluralLabel.toLowerCase()}...`}
              onRowClick={(option) => router.push(toOrgPath(`${meta.href}/${option.id}`))}
              columnOverrides={{
                label: {
                  render: (option) => (
      ...
      <ConfirmDialog
        ...
        description={
          <>
            Delete <strong>{optionToDelete?.label}</strong>? Campaigns using this

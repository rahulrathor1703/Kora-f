...
import {
  useEmailConfigOption,
  useEmailConfigOptions,
} from '@/hooks/useEmailConfigOptions';
import { useOrgPath } from '@/hooks/useOrgPath';
import type { EmailConfigCategory } from '@/lib/api';
import { getEmailConfigCategoryMeta } from '@/lib/email/settings-navigation';
import {
  emailConfigOptionFormSchema,
  slugifyLabel,
  type EmailConfigOptionFormValues,
} from '@/lib/schemas/email-config-option';

interface EmailConfigOptionFormContentProps {
  mode: 'create' | 'edit';
  category: EmailConfigCategory;
  optionId?: string;
}

export default function EmailConfigOptionFormContent({
  mode,
  category,
  optionId,
}: EmailConfigOptionFormContentProps) {
  ...
  const {
    ...
    error: fetchError,
  } = useEmailConfigOption(mode === 'edit' ? (optionId ?? null) : null);
  ...
  } = useForm<EmailConfigOptionFormValues>({
    resolver: zodResolver(emailConfigOptionFormSchema),
    defaultValues: {
      label: '',
      value: '',
      isActive: true,
  ...
  async function onSubmit(values: EmailConfigOptionFormValues) {
    const payload = {
  ...
  return (
    <Stack spacing={3}>
      <SettingsSubPageHeader
        ...
        title={pageTitle}
        description={`Configure a ${meta.label.toLowerCase()} option for the campaign wizard.`}

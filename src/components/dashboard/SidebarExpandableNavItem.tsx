...
export default function SidebarExpandableNavItem({
  ...
  const [userPreference, setUserPreference] = useState<'open' | 'closed' | null>(null);
  const [menuAnchor, setMenuAnchor] = useState<HTMLElement | null>(null);
  const isOpen = userPreference ? userPreference === 'open' : isParentActive;
  ...
  const handleParentClick = useCallback(
    (event: React.MouseEvent<HTMLElement>) => {
      if (isExpanded) {
        const currentlyOpen = userPreference ? userPreference === 'open' : isParentActive;
        setUserPreference(currentlyOpen ? 'closed' : 'open');
        return;
      }

      setMenuAnchor(event.currentTarget);
    },
    [isExpanded, isParentActive, userPreference],
  );

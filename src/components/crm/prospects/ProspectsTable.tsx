...
import {
  getProspectListTableDefaultColumnVisible,
...
export default function ProspectsTable({
  ...
  const tableFields = getProspectTableFields(fields);
  const pipelineStageFieldKey = findPipelineStageField(fields)?.key;
  ...
  const columnOverrides = useMemo(() => {
    ...
    for (const field of tableFields) {
      overrides[field.key] = {
        ...
        defaultVisible: getProspectListTableDefaultColumnVisible(field, fields),
        render: (row) => {
          const isSaving =
          ...
          if (
            canUpdate &&
            onFieldUpdate &&
            pipelineStageFieldKey &&
            field.key === pipelineStageFieldKey
          ) {
            return (
    ...
    return overrides;
  }, [canUpdate, fields, onFieldUpdate, pipelineStageFieldKey, savingCell, tableFields]);
